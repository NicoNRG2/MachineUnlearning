...
class UnlearningTrainer:
    ...
    def extract_features(self, model, dataloader, max_samples=2000):
        ...
        for name, module in model.named_modules():
            # Only keep layer4 (final conv) and fc
        ...
        for layer_name in target_layers:
            ...
            if activations:
                features_dict[layer_name] = torch.cat(activations, dim=0).numpy()
                print(f"  Collected {features_dict[layer_name].shape[0]} samples, "
                      f"dim={features_dict[layer_name].shape[1]}")
    ...
    def compute_svd(self, features):
        ...
        Returns U, S, V for each layer (memory-efficient)
        """
        svd_results = {}
        ...
        for layer_name, feat in tqdm(features.items(), desc="Computing SVD"):
            ...
            # Use randomized SVD for large matrices to save memory
            n_components = min(feat_centered.shape[0], feat_centered.shape[1], 512)
            
            print(f"Layer {layer_name}: Computing SVD with {n_components} components...")
            
            # Compute SVD - use full_matrices=False to save memory
            try:
                ...
                print(f"SVD failed for {layer_name}, using truncated SVD")
    ...
    def compute_projection_matrices(self, svd_results):
        ...
        P = U_k @ U_k^T where k is determined by retained variance
        """
        projection_matrices = {}
        ...
        for layer_name, svd_data in svd_results.items():
            ...
            k = torch.sum(cumsum_var < self.retained_var).item() + 1
            k = max(k, 1)  # At least 1 component
            
            # Get top-k left singular vectors
            U_k = U[:, :k]
            
            # Compute projection matrix P = U_k @ U_k^T
            # This projects onto the subspace spanned by clean data
            P = torch.mm(U_k, U_k.t()).to(self.device)
            
            projection_matrices[layer_name] = P
            
            variance_retained = cumsum_var[k-1].item()
            print(f"Layer {layer_name}: k={k}/{S.shape[0]} "
                  f"({100*k/S.shape[0]:.2f}%), "
    ...
    def get_poisoned_indices(self, train_set):
        ...
        Requires tracking from the dataset
        """
        poison_indices = []
    ...
    def unlearning_loss(self, outputs, labels):
        """
        Custom loss for unlearning poisoned samples
        Loss = L1 + L2
        L1: Maximize uncertainty on poisoned labels
        L2: Maximize entropy over other classes
        """
        ...
        for j in range(batch_size):
            # L1: Reduce confidence on the (wrong) poisoned label
            # We want to maximize -log(p(y)), i.e., minimize p(y)
            logit = logits[j, int(labels[j].item())]
            loss1 += -torch.log(1 - (logit - self.offset))
            
            # L2: Maximize entropy over predictions
            # This prevents the model from being too confident
            entropy = -torch.sum(logits[j] * torch.log(logits[j] + 1e-10))
            loss2 += -entropy  # Negative because we want to maximize entropy
    ...
    def projected_gradient_update(self, model, projection_matrices):
        ...
        Gradient is projected onto the null space of clean data
        to avoid forgetting clean samples
        """
        with torch.no_grad():
            for name, param in model.named_parameters():
                ...
                # Find matching projection matrix
                P_name = self._match_layer_name(name)
                
                if P_name is None or P_name not in projection_matrices:
                    # No projection for this layer, zero out gradient
                    param.grad.data.fill_(0)
                    continue
                
                P = projection_matrices[P_name]
                ...
                # Project gradient: g' = g - P @ g
                # This removes components in the clean data subspace
                if len(reg_grad.shape) >= 2:
                    grad_flat = reg_grad.view(sz, -1)
                    
                    # Check dimension compatibility
                    if grad_flat.shape[0] == P.shape[0]:
                        projected = torch.mm(grad_flat, P)
                        reg_grad = reg_grad - projected.view(param.size())
                
                param.grad.data = reg_grad
    ...
    def unlearn(self, train_set, val_set, poison_indices=None):
        ...
        alpha = np.exp(np.log(self.end_lr / self.start_lr) / self.num_epochs)
        ...
        for epoch in range(self.num_epochs):
            ...
            # Skip first epoch (just evaluate)
            if epoch > 0:
            ...
            history['train_loss'].append(epoch_loss / max(len(poison_loader), 1))
            ...
            # If validation accuracy drops significantly, stop
    ...
    def evaluate(self, loader, name="Test"):
        ...
        # Compute metrics
        accuracy = 100 * np.mean(all_labels == all_preds)
...
def main():
    ...
    parser.add_argument("--unlearn_offset", type=float, default=0.1,
                       help="Offset for unlearning loss")
    ...
    # Save history
    np.save(os.path.join(unlearn_dir, 'history.npy'), history)
