...
class UnlearningTrainer:
    ...
    def extract_features(self, model, dataloader, max_samples=None):
        """
        ...
        for name, module in model.named_modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                if 'layer' in name or 'fc' in name:  # ResNet layers
    ...
    def compute_svd(self, features):
        ...
        for layer_name, feat in tqdm(features.items(), desc="Computing SVD"):
            ...
            svd_results[layer_name] = {
                ...
                'mean': torch.from_numpy(feat.mean(axis=0)).float()
            }
            
            print(f"Layer {layer_name}: Shape {feat.shape}, "
                  f"Singular values: {S.shape[0]}")
    ...
    def unlearn(self, train_set, val_set, poison_indices=None):
        ...
        clean_features = self.extract_features(
            self.model, 
            clean_loader,
            max_samples=5000  # Limit for memory
        )
